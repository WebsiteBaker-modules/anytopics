...
if (!function_exists('display_topics')) {
	...
	function display_topics($showwhat=3, $max_items = 6, $max_chars=600, $max_chars_per=120, $active_only=1,$readmore_text='More..', $latesttopicstitle='<h3>Latest Topics:</h3>', $latestcommentstitle='<h3>Latest Comments:</h3>', $sectiononly=0, $style=0) {
		...
		//Is this better?: Define the constant TOPICS_VIRTUAL_DIRECTORY?
		/*if (!defined('TOPICS_DIRECTORY')) {
			require_once(WB_PATH."/modules/topics/module-settings.default.php");
			require_once(WB_PATH."/modules/topics/module-settings.php");
			if ($topics_virtual_directory)  { define("TOPICS_VIRTUAL_DIRECTORY", $topics_virtual_directory);}		 
		}		
		if (!defined('TOPICS_VIRTUAL_DIRECTORY')) exit("This snippet needs the Topics-Module");*/
		...
		if ($showwhat > 0) {
			...
			$minimum_commentedtime = $t - 120; //Verz?gerung in Sekunden
		...
		if ($showwhat == 0 OR $showwhat > 1) {
			...
			if ($sectiononly == 0) {$query_section = " AND section_id > '0' "; }		
